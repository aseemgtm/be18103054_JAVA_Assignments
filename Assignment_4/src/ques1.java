...
public class ques1 {
	...
	public static void main(String[] args) {
        ...
        System.out.println(replant(crops_mat, 0, 0, 0, n*m, crop_types));
        in.close();
	}

	public static int replant(char[][] crops_mat, int curr_row, int curr_col, int curr_count, int minm_count, char[] crop_types){ 
		// Write your code here 
		// Return the number of replanted crops
		int m = crops_mat.length;
		int n = crops_mat[0].length;
        ...
        if(right_down.contains(crops_mat[curr_row][curr_col]))
        {
            if(present_left_up)
            {
                for(int k=0; k<26; k++)
                {
                    if(!left_up.contains(crop_types[k]) && !right_down.contains(crop_types[k]))
                    {
                        ...
                        if(curr_col<m-1)
                        {
                            minm_count = Math.min(minm_count, replant(new_crops_mat, curr_row, curr_col+1, curr_count+1, minm_count, crop_types));
                        }
                        else if(curr_row<n-1)
                        {
                            minm_count = Math.min(minm_count, replant(new_crops_mat, curr_row+1, 0, curr_count+1, minm_count, crop_types));
                        }
                        else
                        {
            ...
            else
            {
                if(curr_col<m-1)
                {
                    minm_count = Math.min(minm_count, replant(crops_mat, curr_row, curr_col+1, curr_count, minm_count, crop_types));
                }
                else if(curr_row<n-1)
                {
                    minm_count = Math.min(minm_count, replant(crops_mat, curr_row+1, 0, curr_count, minm_count, crop_types));
                }
                else
                {
